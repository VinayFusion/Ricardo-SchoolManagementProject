...
function GetBranchsList() {
    ...
    $.ajax({
        ...
        success: function (dataBranches) {
            ...
            //--if Admin Logged-in
            if (logged_In_UserType_Global == 1) {
                $("#ddlBranch_Filter_ManageExpiredCourseFee").append(res_Branches_Filter);
            }

            //--Get Expired-Course-Fee List Records
            GetExpiredCourseFeeListData();
        },
...
function GetExpiredCourseFeeListData() {

    //var _url = "";

    ////--if Admin Logged-In
    //if (logged_In_UserType_Global == 1) {
    //    _url = "/GetExpiredCourseFeeData";
    //}
    //else {
    //    _url = "/GetExpiredCourseFeeDataByStaff";
    //}

    //GetExpiredCourseFeeListData_FromAPI(_url);
    ...
    $.get("/Admin/GetCurrentDate", null, function (dataCurrentDate) {
        ...
        FilterExpiredCourseFeeDataByBranch();
    });
}
...
function FilterExpiredCourseFeeDataByBranch() {
    StartLoading();

    var _from_date = '';
    var _to_date = '';
    var _branch_id = $("#ddlBranch_Filter_ManageExpiredCourseFee").val();
    ...
    if (logged_In_UserType_Global == 1) {
        var _branch_id_Filter = $("#ddlBranch_Filter_ManageExpiredCourseFee").val();
        if (_branch_id_Filter != undefined && _branch_id_Filter != null && _branch_id_Filter != '' && _branch_id_Filter != '0' && _branch_id_Filter != 0) {
            _url_val = "/GetExpiredCourseFeeDataByBranch?branchID=" + _branch_id_Filter;
        }
        else {
            _url_val = "/GetExpiredCourseFeeData";
        }
    }
    else if (logged_In_UserType_Global == 2) {
        _url_val = "/GetExpiredCourseFeeDataByStaff";
    }
    GetExpiredCourseFeeListData_FromAPI(_url_val, _Params);
}

function GetExpiredCourseFeeListData_FromAPI(_url, _Params) {
    $.ajax({
        type: "POST",
        url: _url,
        headers: {
        ...
        success: function (dataFee) {
            ...
            var _table = $('#tblExpiredCourseFeeData_ManagePendingFee').DataTable();
            _table.destroy();
            ...
            if (dataFee.data.feeData != null) {
                ...
                $('#tblExpiredCourseFeeData_ManagePendingFee').DataTable({
                    data: data,
                    deferRender: true,
                    //scrollY: 200,
                    scrollCollapse: true,
